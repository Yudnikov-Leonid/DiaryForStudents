...
interface PerformanceInteractor {
    ...
    suspend fun analytics(quarter: Int): List<AnalyticsDomain>
    ...
    class Base(
        ...
        override suspend fun analytics(quarter: Int): List<AnalyticsDomain> = try {
            repository.analytics(quarter).map { it.toDomain() }
        } catch (e: Exception) {
